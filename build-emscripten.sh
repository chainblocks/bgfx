...
# if uname | grep -qiE "mingw|msys"; then
#   sed -i 's/shadercRelease.exe/shaderc.html/g' shaderc.make
#   sed -i 's/shadercDebug.exe/shaderc.html/g' shaderc.make
# else
#   sed -i 's/shadercRelease/shaderc.html/g' shaderc.make
#   sed -i 's/shadercDebug/shaderc.html/g' shaderc.make
# fi
...
ARCH="-s MODULARIZE=1 -s 'EXPORT_NAME="shaderc"' -s 'EXTRA_EXPORTED_RUNTIME_METHODS=["FS"]' -s ALLOW_MEMORY_GROWTH=1 $USE_LTO"
